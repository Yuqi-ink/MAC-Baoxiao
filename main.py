...
def build_invoice_merge_sequence(
    ...
    for f in hotel_inv_list:
        sequence.append((f.getvalue(), f.name))
    if inv_out_p1:
    ...
    for f in ins_list_p1:
        sequence.append((f.getvalue(), f.name))
    for f in ins_list_p2:
        sequence.append((f.getvalue(), f.name))
    for f in other_inv_list:
        sequence.append((f.getvalue(), f.name))
    return sequence
...
if st.button("开始生成报销材料包", use_container_width=True):
    ...
    temp_invoice_summary_pdf = tempfile.NamedTemporaryFile(suffix=".pdf", delete=False).name
    ...
    try:
        ...
        with st.spinner("????????? PDF?"):
            merge_files_to_pdf(invoice_merge_sequence, temp_invoice_summary_pdf)
        ...
        with zipfile.ZipFile(temp_outer_zip_path, "w", zipfile.ZIP_DEFLATED) as zout:
            ...
            zout.write(temp_merged_pdf, arcname="证明材料汇总打印.pdf")
            zout.write(temp_invoice_summary_pdf, arcname="电子发票汇总打印.pdf")
