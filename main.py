...
def build_invoice_merge_sequence(
    hotel_inv_list,
    inv_out_p1,
    inv_out_p2,
    inv_return_p1,
    ...
    sequence: list[tuple[bytes, str]] = []
    for inv_hotel_f in hotel_inv_list:
        sequence.append((inv_hotel_f.getvalue(), inv_hotel_f.name))
    if inv_out_p1:
        sequence.append((inv_out_p1.getvalue(), inv_out_p1.name))
    if inv_out_p2:
        sequence.append((inv_out_p2.getvalue(), inv_out_p2.name))
    if inv_return_p1:
        sequence.append((inv_return_p1.getvalue(), inv_return_p1.name))
    if inv_return_p2:
        sequence.append((inv_return_p2.getvalue(), inv_return_p2.name))
    for ins_f in ins_list_p1:
        sequence.append((ins_f.getvalue(), ins_f.name))
    for ins_f in ins_list_p2:
        sequence.append((ins_f.getvalue(), ins_f.name))
    for oth_f in other_inv_list:
        sequence.append((oth_f.getvalue(), oth_f.name))
    return sequence
...
if st.button("开始生成报销材料包", use_container_width=True):
    ...
    invoice_merge_sequence = build_invoice_merge_sequence(
        hotel_inv_list,
        inv_out_p1,
        inv_out_p2,
        inv_return_p1,
    ...
    invoice_summary_pdf_buf = io.BytesIO()
    with st.spinner("????????? PDF?"):
        merge_files_to_pdf(invoice_merge_sequence, invoice_summary_pdf_buf)
    invoice_summary_pdf_bytes = invoice_summary_pdf_buf.getvalue()
    invoice_summary_pdf_buf.close()
    ...
    with zipfile.ZipFile(outer_zip_buf, "w", zipfile.ZIP_DEFLATED) as zout:
        ...
        zout.writestr("??????.pdf", invoice_summary_pdf_bytes)
